...
SELECT
	...
	CONCAT(
		ROUND(
			100.0 * SUM(P.PRICE * OD.QUANTITY) / SUM(SUM(P.PRICE * OD.QUANTITY)) OVER (),
			2
		),
		'%'
	) AS PCT_CONTRIBUTION
...
SELECT
	...
	SUM(TOTAL_REVENUE) OVER (
		ORDER BY Month_over_Month
			
	) AS REVENUE_TRND
FROM
	(
		SELECT
			to_char(o.order_date,'Month') as Month_over_Month,
			SUM(P.PRICE * OD.QUANTITY) AS TOTAL_REVENUE
		FROM
			PIZZAS AS P
			JOIN ORDER_DETAILS AS OD ON P.PIZZA_ID = OD.PIZZA_ID
...
SELECT
	CASE
		WHEN TOTAL_SPEND > 82500 THEN 'High_value'
		ELSE 'Regular'
	END AS SEGMENT,
